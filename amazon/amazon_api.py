import requests
from ..schemas.schemas import SearchResponse, Product, ProductPrice,ProductDetail,ProductDetailResponse
import os
...
def search_products(query: str) -> SearchResponse:
    ...
    for item in shopping_results:
        product = Product(
            ...
            price=ProductPrice(
                value=float(item.get('price', {}).get('value', 0) if isinstance(item.get('price'), dict) else 0),
                currency=item.get('price', {}).get('currency', '') if isinstance(item.get('price'), dict) else '',
                raw=item.get('price', {}).get('raw', '') if isinstance(item.get('price'), dict) else str(item.get('price', ''))
            ),
...
def get_product_details(asin: str) -> ProductDetailResponse:
    ...
    product_detail = ProductDetail(
        ...
        price=ProductPrice(
            value=float(product_data.get('buybox', {}).get('price', {}).get('value', 0)),
            currency=product_data.get('buybox', {}).get('price', {}).get('currency', ''),
            raw=product_data.get('buybox', {}).get('price', {}).get('raw', '')
        ) if product_data.get('buybox', {}).get('price') else None,
        rating=product_data.get('rating', None),
        ratings_total=product_data.get('reviews', None),
        reviews=[],  
        link=product_data.get('link', ''),
        brand=product_data.get('brand_store', {}).get('text', '') if product_data.get('brand_store') else '',
