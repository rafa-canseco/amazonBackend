...
from .schemas import SearchRequest, SearchResponse,ProductDetailRequest,ProductDetailResponse,UserData,CartItem,Cart,CreateOrderRequest,CreateOrderResponse,Order,OrderItem, UpdateOrderStatusRequest
from .amazon import search_products,get_product_details
...
def verify_admin_token(credentials: HTTPAuthorizationCredentials = Depends(security)):
    try:
        ...
        wallet_address = payload.get("wallet_address")  # Asumiendo que Privy incluye la dirección del wallet en el token
...
async def remove_from_cart(user_id: str, asin: str):
    try:
        supabase.table("cart_items").delete().eq("user_id", user_id).eq("asin", asin).execute()
        return await get_cart(user_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def update_cart_item_quantity(user_id: str, asin: str, quantity: int):
    try:
        if quantity > 0:
            supabase.table("cart_items").update({"quantity": quantity}).eq("user_id", user_id).eq("asin", asin).execute()
        else:
...
async def create_order(order_details: CreateOrderRequest):
    try:
        ...
        # Acceder al primer elemento de la lista de datos y obtener el ID generado por Supabase
        order_id = order_response.data[0]['id']

        # Insertar los items de la orden
        order_items = [
            {
                "order_id": order_id,  # Usar el ID generado por Supabase
                "asin": item.asin,
                "quantity": item.quantity,
                "price": item.price,
                "title": item.title,
        ...
        supabase.table("cart_items").delete().eq("user_id", order_details.user_id).execute()
        ...
        order_data = supabase.table("orders").select("*, order_items(*)").eq("id", order_id).single().execute()
        ...
        return Order(
            ...
            total_amount_usd=float(order_data.data['total_amount_usd']) if order_data.data['total_amount_usd'] is not None else None,
    ...
    try:
        ...
        for order_data in orders_data.data:
            ...
            order = Order(
                ...
                total_amount_usd=float(order_data['total_amount_usd']) if order_data['total_amount_usd'] is not None else None,
                status=order_data['status'],
                created_at=order_data['created_at'],
                items=order_items,
                full_name=order_data['full_name'],
                ...
                phone=order_data['phone'],
                delivery_instructions=order_data['delivery_instructions'],
                shipping_guide=order_data.get('shipping_guide', "Generando orden de envío"),
...
async def update_order_status(order_id: str, request: UpdateOrderStatusRequest, admin_id: str = Depends(verify_admin_token)):
    try:
        response = supabase.table("orders").update({
            "status": "shipped",
            "shipping_guide": request.shippingGuide
        }).eq("id", order_id).execute()

        if not response.data:
            raise HTTPException(status_code=404, detail="Order not found or no update made")
...
async def get_latest_exchange_rate():
    try:
        # URL para obtener el dato más reciente de la serie de tipo de cambio.
        url = "https://www.banxico.org.mx/SieAPIRest/service/v1/series/SF43718/datos/oportuno"
        
        # Realizar la solicitud a la API de Banxico
        headers = {
        ...
        async with httpx.AsyncClient() as client:
            ...
            response.raise_for_status()  # Lanzar un error si la respuesta no es 200
        ...
        latest_data = data.get("bmx", {}).get("series", [{}])[0]  # Acceder a la primera serie

        if not latest_data:
            raise HTTPException(status_code=404, detail="No data found for the given series.")
        ...
        latest_value = datos[0]  # Considerar el último dato
